...
def consume_temperature_messages(temp_consumer: kafka_handler.KafkaConsumer, shared_data: dict, lock: threading.Lock):
    for temp_msg in temp_consumer:
        ...
        with lock:
            ...
            if shared_data['humidity'] is not None:
                ...
                shared_data['producer'].send('perceived_temperature', json.dumps({'perceived_temp': perceived_temp}))
                logging.info(f"Sent perceived temperature: {perceived_temp}")
                shared_data['humidity'] = None

def consume_humidity_messages(humidity_consumer: kafka_handler.KafkaConsumer, shared_data: dict, lock: threading.Lock):
    for humidity_msg in humidity_consumer:
        ...
        with lock:
            ...
            if shared_data['temperature'] is not None:
                ...
                shared_data['producer'].send('perceived_temperature', json.dumps({'source': 'kafka', 'perceived_temp': perceived_temp}))
                logging.info(f"Sent perceived temperature: {perceived_temp}")
...
def main():
    ...
    temp_thread = threading.Thread(target=consume_temperature_messages, args=(temp_consumer, shared_data, lock))
    humidity_thread = threading.Thread(target=consume_humidity_messages, args=(humidity_consumer, shared_data, lock))
